...
router.post("/compare", validateCompareRequest, async (req, res) => {
  try {
    ...
    const beforeResp = beforeUrl ? await axios.get(beforeUrl, { responseType: "arraybuffer" })
                                : await axios.get(`${process.env.BACKEND_URL}/api/images/gibs?forest=${encodeURIComponent(forest)}&date=${beforeDate}`, { responseType: "arraybuffer" });
    const afterResp = afterUrl ? await axios.get(afterUrl, { responseType: "arraybuffer" })
                                : await axios.get(`${process.env.BACKEND_URL}/api/images/gibs?forest=${encodeURIComponent(forest)}&date=${afterDate}`, { responseType: "arraybuffer" });
